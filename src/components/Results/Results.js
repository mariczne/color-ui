...
const Results = (props) => {
  const {
    imageUrl, colors, isLoadingResults, errors,
  } = props;
      ...
      <Grid columns={2} stackable stretched>
        <Grid.Column width={10}>
          <Segment style={{ display: 'flex', alignItems: 'center', minHeight: '100px' }}>
            <Image alt="Image supplied by the user" src={imageUrl} rounded fluid centered style={{ maxHeight: '450px', width: 'auto' }} />
          </Segment>
        </Grid.Column>
        <Grid.Column width={6} stretched>
          ...
          <Segment style={{ height: '50%', textAlign: 'center' }} loading={isLoadingResults}>
            <ColorsList colors={colors} />
            <small><i>tip: click on color to copy it to clipboard!</i></small>
          </Segment>
        </Grid.Column>
      </Grid>
    </Segment>
